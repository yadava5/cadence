...
/**
 * Development-only auth toggle: seeds a mock JWT session so the app shell can
 * be driven without a backend, and logs out again. Draggable so it can be moved
 * off whatever it happens to cover.
 *
 * This module lives in its own file and is rendered ONLY behind
 * `{import.meta.env.DEV && <DevAuthToggle />}` in src/App.tsx. Vite replaces
 * that flag with a literal `false` in production builds, so the JSX is dropped,
 * App.tsx's import of this module is left unreferenced, and Rollup tree-shakes
 * it away — the "Mock Login" affordance and the `mock-access-token` literal
 * exist in dev builds only (asserted by grepping dist/). The matching skip in
 * src/hooks/useAuthGuard.ts is gated the same way; the two must stay in step.
 */
