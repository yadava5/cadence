...
router.post(
  ...
  validateBody(changePasswordSchema) as unknown as RequestHandler,
  async (
      ...
      if (error instanceof Error) {
        switch (error.message) {
          ...
          case 'CANNOT_UNLINK_ONLY_AUTH_METHOD':
            ...
            errorMessage =
              'Cannot unlink Google account as it is the only authentication method. Please set a password first.';
            statusCode = 400;
            break;
        }
      }
...
router.post(
  '/refresh',
  refreshTokenRateLimit as unknown as RequestHandler,
  validateRefreshToken as unknown as RequestHandler,
  async (
    req: Request & { user?: { id: string; email: string } },
    res: Response
  ) => {
      ...
      if (isTokenReuse) {
        return res.status(401).json({
          ...
          error: {
            ...
            message:
              'Security breach detected. All tokens have been invalidated.',
            timestamp: new Date().toISOString(),
          },
        });
      }
...
router.get(
  '/me',
  authenticateToken as unknown as RequestHandler,
  (req: Request & { user?: { id: string; email: string } }, res: Response) => {
    const user = req.user!;
